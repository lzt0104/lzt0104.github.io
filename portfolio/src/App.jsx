          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            <button
              ...
              onMouseEnter={(e) => {
                ...
                e.target.style.boxShadow = `0 4px 12px ${COLORS.success}60`;
              }}
              onMouseLeave={(e) => {
            ...
            <button
              ...
              onMouseLeave={(e) => {
                e.target.style.borderColor = `${COLORS.text}40`;
              }}
            >
              取消
            </button>
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: 'clamp(1rem, 3vw, 2rem)' }}>
      <div style={{
        ...
        <p style={{
          ...
          我是108課綱第一屆的「白老鼠」，熱愛探索各種技術領域並挑戰自我。曾在各大社群活躍，對教育與技術有深厚熱情。現在是一位懷抱夢想的資管系學生，專注於人工智慧與資訊系統開發。我的專長橫跨多個技術領域，正在尋找屬於自己的道路。
        </p>
...
const BlogPage = ({ isAdmin }) => {
  ...
  const loadPosts = async () => {
    try {
      setLoading(true);
      const result = await window.storage.list('blog:', true);
      if (result && result.keys) {
        ...
        setPosts(loadedPosts.filter(p => p !== null).sort((a, b) => 
          new Date(b.date) - new Date(a.date)
        ));
      }
    } catch (error) {
      console.log('載入文章:', error);
      setPosts([]);
    } finally {
      setLoading(false);
    }
  ...
  const savePost = async () => {
    ...
    try {
      const post = {
        id: isEditing ? selected.id : `post_${Date.now()}`,
        title: formData.title,
        content: formData.content,
        tags: formData.tags.split(',').map(t => t.trim()).filter(t => t),
        date: isEditing ? selected.date : new Date().toISOString().split('T')[0],
        updated: new Date().toISOString().split('T')[0]
      };

      await window.storage.set(`blog:${post.id}`, JSON.stringify(post), true);
      await loadPosts();
      setIsCreating(false);
      setIsEditing(false);
  ...
  const deletePost = async (id) => {
    ...
    try {
      await window.storage.delete(`blog:${id}`, true);
      await loadPosts();
      setSelected(null);
    } catch (error) {
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1rem', flexWrap: 'wrap', gap: '1rem' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{
                          ...
                          發布: {selected.date} {selected.updated !== selected.date && `| 更新: ${selected.updated}`}
                        </div>
                      </div>
                      {isAdmin && (
...
export default function App() {
  ...
  // 檢查 localStorage 是否有管理員 session
  useEffect(() => {
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        ...
        {page === 'blog' && <BlogPage isAdmin={isAdmin} />}
